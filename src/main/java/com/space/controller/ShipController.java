...
import static com.space.util.ShipHelper.validateShipFields;
...
public class ShipController {
    ...
    public ResponseEntity<ShipDto> updateShip(@PathVariable("id") Object id, @RequestBody ShipDto shipDto) {
        log.info("Update ship");
        try {
            Long shipId = validateId(id);
            if (validateShip(shipDto)) {
                shipDto.setId(shipId);
                Ship ship = shipService.update(shipDto);
                return new ResponseEntity<>(EntityMapper.toShipDto(ship), HttpStatus.OK);
            }
            return new ResponseEntity<>(EntityMapper.toShipDto(shipService.get(shipId)), HttpStatus.OK);
